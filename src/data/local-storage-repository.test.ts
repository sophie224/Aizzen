...
describe('LocalStorageRepository.getState', () => {
  ...
  it('migrates a stored legacy v7 payload and rewrites it at v8', async () => {
    ...
    expect((stored as { schemaVersion: number }).schemaVersion).toBe(8)
  })
...
describe('AppDataStore mutation pipeline', () => {
  ...
  it('loads state through the repository', async () => {
    ...
    expect(store.getSnapshot().state?.schemaVersion).toBe(8)
  })
